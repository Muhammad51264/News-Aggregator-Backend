...
router.post("/edit/:id",upload.single("img"), async function (req, res) {
  ...
  try {
    ...
    if(!updatedImg){
      ...
      updatedImg = user.img;
      await News.updateOne({_id : newsId},{category:news.category,title:news.title,desc:news.desc,img:updatedImg,date:news.date});
      ...
      cloudinary.uploader
        ...
        await News.updateOne({_id : newsId},{category:news.category,title:news.title,desc:news.desc,img:imgURL,date:news.date});
          // fs.unlinkSync(`Pictures/NewsPictures/${uploadedImage.filename}.jpg`);
          return res.json({ status: "Success", message:"edited successfully with image" });
        })
        .catch((err) => {
          console.log("failed");
          console.log("error", JSON.stringify(err, null, 2));
